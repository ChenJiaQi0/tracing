...
public class SysThirdAccountServiceImpl extends ServiceImpl<SysThirdAccountMapper, SysThirdAccount> implements ISysThirdAccountService {
    ...
    @Override
    public SysThirdAccount saveThirdAndSysUser(SysThirdAccount sysThirdAccount, SysUser sysUser) {
        String thirdUserId = sysThirdAccount.getThirdUserId();
        String thirdType = sysThirdAccount.getThirdType();
        SysThirdAccount dbSysThirdAccount = getOneByThirdUserId(thirdUserId, thirdType);

        // 未找到第三方关联
        if (ObjectUtil.isEmpty(dbSysThirdAccount)) {
            // 需要创建用户
            if (StrUtil.isBlank(sysUser.getId())) {
                ...
                sysUser.setUsername(thirdUserId);
                ...
                sysUser.setRelTenantIds("0");
                // 获取免登角色
                SysRole sysRole = sysRoleService.lambdaQuery().eq(SysRole::getRoleCode, CommonConstant.DEFAULT_ROLE_CODE).one();
                roleId = sysRole.getId();
                // 获取顶级部门
                SysDepart defaultDepart = sysDepartService.list().get(0);
                departId = defaultDepart.getId();

                String salt = oConvertUtils.randomGen(8);
                sysUser.setSalt(salt);
                String passwordEncode = PasswordUtil.encrypt(sysUser.getUsername(), CommonConstant.DEFAULT_USER_PASSWORD, salt);
                sysUser.setPassword(passwordEncode);
                sysUser.setStatus(CommonConstant.STATUS_1_INT);
                sysUser.setDelFlag(CommonConstant.STATUS_0_INT);
                sysUserService.saveUser(sysUser, roleId, departId, "0", true);
            }

            if (ObjectUtil.isNotEmpty(sysUser) && StrUtil.isNotBlank(sysUser.getId())) {
                // 保存第三方
                sysThirdAccount = new SysThirdAccount();
                sysThirdAccount.setSysUserId(sysUser.getId());
                sysThirdAccount.setThirdUserId(thirdUserId);
                sysThirdAccount.setThirdUserUuid(thirdUserId);
                sysThirdAccount.setThirdType(thirdType);
                sysThirdAccount.setRealname(sysUser.getRealname());
                sysThirdAccount.setStatus(Integer.valueOf(CommonConstant.STATUS_1));
