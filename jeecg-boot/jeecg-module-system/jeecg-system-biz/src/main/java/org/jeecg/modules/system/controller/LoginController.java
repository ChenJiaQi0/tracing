...
import org.jeecg.modules.system.config.WxMaConfiguration;
...
//	}
	...
	@GetMapping("/wx-mini/checkRegister")
	public Result<JSONObject> checkRegister(@RequestParam("code") String code) throws WxErrorException {
		final WxMaService wxService = WxMaConfiguration.getMaService();
		WxMaJscode2SessionResult session = wxService.getUserService().getSessionInfo(code);
		String openId = session.getOpenid();
		log.info("当前登录人openId: {}", openId);

		SysUser dbUser = sysUserService.getUserByName(openId);
		Boolean isRegister = !ObjectUtil.isEmpty(dbUser);
	...
	public Result<JSONObject> wxLogin(@RequestBody SysLoginModel sysLoginModel, HttpServletRequest request) {
		sysLoginModel.setThirdType("wechat_mini");
		Result<JSONObject> result = thirdAccountLoginUtil(sysLoginModel, request);
		return result;
	}
	 ...
	 * @throws Exception
	 */
	private Result<JSONObject> thirdAccountLoginUtil(SysLoginModel sysLoginModel, HttpServletRequest request) {
		Result<JSONObject> result = new Result<>();
		// 查询第三方关联表
		SysThirdAccount sysThirdAccount = sysThirdAccountService.getOneByThirdUserId(sysLoginModel.getThirdUserId(), sysLoginModel.getThirdType());

		// 系统用户
		SysUser sysUser = sysUserService.getUserByName(sysLoginModel.getThirdUserId());
		if (ObjectUtil.isNotEmpty(sysThirdAccount)) {
			// 校验用户有效性
			result = sysUserService.checkUserIsEffective(sysUser);
			if (!result.isSuccess()) {
				return result;
			...
			BeanUtil.copyProperties(sysLoginModel, sysThirdAccount);
			sysThirdAccount = sysThirdAccountService.saveThirdAndSysUser(sysThirdAccount, sysUser);
		}
		log.info("sysThirdAccount信息：{}", sysThirdAccount);
		if (StrUtil.isNotBlank(sysThirdAccount.getId())) {
		...
		// 用户信息
		userInfo(sysUser, result, request, CommonConstant.CLIENT_TYPE_APP);
		// 添加日志
		LoginUser loginUser = new LoginUser();
		BeanUtils.copyProperties(sysUser, loginUser);

		String successMessage = "用户名: " + sysUser.getUsername() + ",登录成功[第三方——移动端]！";
		baseCommonService.addLog(successMessage, CommonConstant.LOG_TYPE_1, null, loginUser);
		return result;
	}
}