...
<script lang="ts" setup>
  ...
  const props = defineProps({
    ...
    // 选中节点的显示策略：SHOW_ALL(显示所有选中节点)、SHOW_PARENT(只显示父节点)、SHOW_CHILD(只显示子节点)
    showCheckedStrategy: propTypes.string.def('SHOW_ALL'),
   ...
   * @param arr
   */
  // function findChildrenNode(data, arr){    let val = props.value;    // 将逗号分隔的字符串转换为数组    let values = Array.isArray(val) ? val : (val ? val.split(',') : []);    if(data && data.length){      for(let item of data){        // 检查当前节点是否匹配        if(values.includes(item.value)){          arr.push({            key: item.key,            value: item.value,            label: item.label||item.title          })        }        // 递归查找子节点        findChildrenNode(item.children, arr)      }    }  }
  function findChildrenNode(data, arr){
    let val = props.value;
    // 将逗号分隔的字符串转换为数组
    let values = Array.isArray(val) ? val : (val ? val.split(',') : []);
    if(data && data.length){
